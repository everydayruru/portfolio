...
(async e => {
  ...
  try {
    json = await axios.get('http://localhost:3000/news');
  } catch (e) {
    alert('데이터 전송 중 에러가 발생하였습니다.');
    return;
  }

  {let {data} = json; getArtWrap(data);}


  try {
    json = await axios.get('http://localhost:3000/main');
  } catch (e) {
    alert('데이터 전송 중 에러가 발생하였습니다.');
    return;
  }

  {let {data} = json; getNtcWrap(data);}


  try {
    json = await axios.get('http://localhost:3000/social');
  } catch (e) {
    alert('데이터 전송 중 에러가 발생하였습니다.');
    return;
  }

  {let {data} = json; getSclWrap(data);}
})();